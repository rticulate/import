...
The imported objects are placed in a
...
**Contents of "[some_module.R](https://raw.githubusercontent.com/torfason/import/docs/man/examples/some_module.R)":**
```{r, eval=FALSE}
...
clearly distinguish them from other named arguments. The `import::into`
and `import::from` accept the same parameters and achieve the same
result. The the choice between them a matter of preferece).
...
[plusone_module.R](https://raw.githubusercontent.com/torfason/import/docs/man/examples/plusone_module.R):

```{r, eval = FALSE}
url <- "https://raw.githubusercontent.com/torfason/import/docs/man/examples/plusone_module.R"
...
A frequent pattern in python imports packages under an alias; all subsequent use of the imported objects then explitly includes the alias:
...
In order to achieve this functionality with the import package, use `.into={new.env()}` which assign to a new environment without attachihng it. `import::from()` returns this environment, so it can be assigned toa variable:
