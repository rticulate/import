...
One can also specify which names to use in the search path 
and use several to group imports:
...
clearly distinguish them from other named arguments. Be aware that
hidden objects (those with names prefixed by a period) are supported.
Take care to avoid name clashes with argument names. The `import::into`
and `import::from` accept the same parameters and achieve the same
result. The the choice between them a matter of preference).
...
The `.into` parameter can be surrounded with `{}`, which leads it to be
interpreted as a variable or function, instead of as the name of an
environment in the search path. Examples include:
...
Lastly, the `.into` parameter can be input as a symbol, allowing dynamic environment naming.

```{r, eval = FALSE}
# define a variable representing the environment name
env_name <- "operators"

# this will import into a new "operators" environment
import::into(env_name, "%>%", "%$%", .from = magrittr)

# define a variable representing the environment name
env_name <- "datatools"

# shows that .into is unaffected by .character_only
import::from("dplyr", .into = env_name, "arrange", .character_only = TRUE)
```
...
The `import` package uses non-standard evaluation in order to allow the 
...
`...` parameters is disabled. Instead, the parameters are processed as
character vectors containing the relevant values. (`.character_only=TRUE`
does not effect the evaluation of `.into`, which still uses non-standard
evaluation. Other parameters are always evaluated in a standard way.) 

It is useful to examine some examples of how this can be helpful.
